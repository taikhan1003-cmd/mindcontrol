...
// Fallback response generator if Gemini key is not configured or on network error
function generateFallbackResponse(content: string, emotion: string) {
  const emotionMap: Record<string, { reply: string; songTitle: string; artist: string; activity: string }> = {
    joy: {
...
// API Route for AI Emotion Analysis & Counselor Reply
app.post('/api/analyze-entry', async (req, res) => {
  const { content, emotion, date } = req.body;

  if (!content || !emotion) {
    return res.status(400).json({ error: 'Diary content and emotion selection are required.' });
  }

  const aiClient = getGeminiClient();

  if (!aiClient) {
    console.log('Gemini API key not found or placeholder. Returning empathetic fallback response.');
    return res.json(generateFallbackResponse(content, emotion));
  }
...
`;
    ...
    // On failure, return graceful fallback
    return res.json(generateFallbackResponse(content, emotion));
  }
});
