...
function generateFallbackResponse(content: string, emotion: string) {
  const emotionMap: Record<string, { reply: string; songTitle: string; artist: string; activity: string; category: string }> = {
    joy: {
...
export default async function handler(req: any, res: any) {
  ...
  if (!apiKey || apiKey === 'MY_GEMINI_API_KEY') {
    return res.status(200).json(generateFallbackResponse(content, emotion));
  }
...
`;
    ...
    console.error('Error calling Gemini API in serverless handler:', err);
    return res.status(200).json(generateFallbackResponse(content, emotion));
  }
}
